...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const farmYield = document.getElementById('farm-yield');
    const farmDimensions = document.getElementById('farm-dimensions');
    const farmDifficulty = document.getElementById('farm-difficulty');
    const farmMaterials = document.getElementById('farm-materials');
    ...
    function renderAdminTable(filterKeyword = '') {
        ...
        if (filterKeyword.trim() !== '') {
            ...
            filtered = currentItems.filter(i => 
                ...
                (i.version && i.version.toLowerCase().includes(q)) ||
        ...
        filtered.forEach(item => {
            ...
            const infoText = item.farmInfo && item.farmInfo.yield 
                ? `<span style="color:var(--emerald);font-weight:700;">${item.farmInfo.yield}</span>` 
            ...
            tr.innerHTML = `
                ...
                <td><code>${item.version || '1.21+'}</code></td>
                <td>${infoText}</td>
                <td>
    ...
    function openEditModal(item) {
        ...
        if (item.farmInfo) {
            farmYield.value = item.farmInfo.yield || '';
            farmDimensions.value = item.farmInfo.dimensions || '';
            farmDifficulty.value = item.farmInfo.difficulty || 'Trung bình';
            farmMaterials.value = item.farmInfo.materials || '';
            farmNotes.value = item.farmInfo.notes || '';
        } else {
            farmYield.value = '';
            farmDimensions.value = '';
            farmDifficulty.value = 'Trung bình';
            farmMaterials.value = '';
            farmNotes.value = '';
        }
    ...
    resourceForm.addEventListener('submit', (e) => {
        ...
        const version = itemVersion.value.trim();
        ...
        if (selectedCategory === 'litematica') {
            farmInfo = {
                yield: farmYield.value.trim() || 'N/A',
                dimensions: farmDimensions.value.trim() || 'N/A',
                difficulty: farmDifficulty.value.trim() || 'Trung bình',
                materials: farmMaterials.value.trim() || 'Chưa cập nhật',
...
`;
    ...
    async function pushToGitHub() {
        ...
        try {
            ...
            const payload = {
                message: `Cập nhật dữ liệu Resource Hub [${new Date().toLocaleString('vi-VN')}]`,
